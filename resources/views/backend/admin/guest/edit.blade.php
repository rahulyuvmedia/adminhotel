            ...
            <div class="d-flex flex-wrap justify-content-between align-items-center  p-4">
                ...
                <div class="col-lg-2 col-md-3 col-sm-12 mb-4">
                    ...
                    <input type="text" name='roomNumber' class='form-control'
                        value="{{ isset($model->reservations->room->roomNumber) ? $model->reservations->room->roomNumber : 'No Room Assigned' }}"
                        readonly />
                    @error('roomNumber')
                ...
                <div class="col-lg-2 col-md-3 col-sm-12 mb-4">
                    ...
                    <select id="member" name="member" class="form-control">
                        @for ($i = 1; $i <= 15; $i++) <option value="{{ $i }}" @if ($i==$model->member) selected @endif>
                            {{ $i }}</option>
                            @endfor
                    </select>
                    @error('member')
                ...
                <div class="col-lg-2 col-md-3 col-sm-12 mb-4">
                    ...
                    <select id="child" name="child" class="form-control">
                        @for ($i = 1; $i <= 15; $i++) <option value="{{ $i }}" @if ($i==$model->child) selected @endif>
                            {{ $i }}</option>
                            @endfor
                    </select>
                    @error('child')
                ...
                <div class="col-lg-3 col-md-3 col-sm-12 mb-4">
                    <div class="form-group col-lg-6">
                        <label class="form-check">
                            @if($guest->hasInvoice())
                            <input type="checkbox" class="form-check-input" name="payment" checked>
                            <label class="form-check-label">Payment</label>
                            @else
                            <label class="form-check-label payment-pending">
                    Payment is Pending<span class="loading-dots"></span>
                </label>
